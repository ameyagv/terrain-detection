...
def normalize_data(input_data, data_columns):
    ...
    standard_scaler = standard_scaler.fit(input[data_columns])
...
def match_frequency(X,y):
    ...
    X_df = X_df.iloc[: -(X_df.shape[0] - upsampled_y.shape[0]), :]
...
def get_ts_format(X_list, y_list, n_steps, step_size):
    ...
    for each in range(len(y_list)):
        ...
        X_values = list(X.columns.values)
        X = normalize_data(X, X_values)
        X, y = get_windowed_data(X, y, n_steps, step_size)
        list_X.append(X)
        list_y.append(y)
